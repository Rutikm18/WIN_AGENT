...
param(
    ...
    [string] $DataDir       = "",
    [string] $ManagerUrl    = "http://YOUR_MANAGER_IP:8080",
    [string] $TlsVerify     = "false",
    [string] $EnrollToken   = "",
...
if ($ManagerUrl -like "*YOUR_MANAGER*") {
    Write-Warning "ManagerUrl is still set to the placeholder. Edit agent.toml after install."
}
...
# [collection] — all 23 sections (22 standard + eventlog Windows-only)
$sections = [ordered]@{
    metrics     = @{ e=$true;  i=10    }
