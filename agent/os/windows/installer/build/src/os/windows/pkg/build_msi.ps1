<#
.SYNOPSIS
    Build the AttackLens Agent Windows MSI package (PyInstaller EXE approach).

.DESCRIPTION
    Full pipeline:
      1. Build standalone EXEs via PyInstaller  (build_exe.ps1)
      2. Locate or download WiX Toolset (v4 preferred, v3 fallback)
      3. Compile jarvis-agent.wxs → attacklens-agent-<version>-x64.msi
      4. Optionally sign the MSI with Authenticode

    Output: agent\os\windows\pkg\dist\attacklens-agent-<version>-x64.msi

    NOTE: For the Python source-based MSI (no PyInstaller, recommended for
    development), use the installer in:
      agent\os\windows\installer\build_msi.ps1

.PARAMETER Version
    Semantic version string embedded in MSI and EXEs. Default: 2.0.0
...
.EXAMPLE
    ...
    .\build_msi.ps1 -Version "2.0.0" -SignIdentity "CN=AttackLens Inc"
...
.EXAMPLE
    ...
    msiexec /i dist\attacklens-agent-2.0.0-x64.msi /qn `
...
$MsiOut         = "$DistDir\attacklens-agent-$Version-x64.msi"

# ── Banner ────────────────────────────────────────────────────────────────────
Write-Host ""
Write-Host "  AttackLens Agent MSI Builder (PyInstaller)" -ForegroundColor Cyan
Write-Host "  Version: $Version" -ForegroundColor Cyan
Write-Host "  Note: for source-based MSI see installer\build_msi.ps1" -ForegroundColor DarkGray
Write-Host ""
