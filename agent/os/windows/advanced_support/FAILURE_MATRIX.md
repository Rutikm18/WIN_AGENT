...
| GUI manager becomes blank after successful 2.0.21 install | MSI log has `MANAGER_URL` in client/server but generator says `manager_source=none`; protected TOML stays empty or preserved | 2.0.24 captures on `AttackLensSecurityDlg.Next`, builds from authoritative Secure server properties, formats hidden `[CA_WriteConfig]` into the deferred EXE command, and fails closed if the GUI manager is absent | Upgrade through the 2.0.24 GUI; if recovery is needed first, run installed `configure-manager.ps1` elevated |
